...
class CactusService {
  ...
  final ValueNotifier<String?> stagedAssetPath = ValueNotifier(
    null,
  ); // For image picker display
  ...
  Future<void> sendMessage(String userInput) async {
    ...
    if (_lastInteractionWasMultimodal != isCurrentlyMultimodal) {
      debugPrint(
        "Mode switch: ${_lastInteractionWasMultimodal ? 'multimodal' : 'text'} -> ${isCurrentlyMultimodal ? 'multimodal' : 'text'}, clearing conversation state",
      );
    ...
    try {
      if (isCurrentlyMultimodal) {
        ...
        if (completionHistory.isNotEmpty &&
            completionHistory.last.role == 'assistant' &&
            completionHistory.last.content.isEmpty) {
        ...
        final completionParams = CactusCompletionParams(
          ...
          onNewToken: (String token) {
            ...
            if (token.isNotEmpty) {
              ...
              final List<ChatMessage> streamingMessages = List.from(
                chatMessages.value,
              );
        ...
        final result = await _cactusContext!.multimodalCompletion(
          completionParams,
          [imagePathToSend],
        );
        ...
        if (finalMessages.isNotEmpty &&
            finalMessages.last.role == 'assistant') {
          finalMessages[finalMessages.length - 1] = ChatMessage(
            role: 'assistant',
            content:
                finalCleanText.isNotEmpty
                    ? finalCleanText
        ...
        final result = await _cactusContext!.continueConversation(
          userInput,
          maxTokens: 256,
        );
        ...
        debugPrint(
          "[PERFORMANCE] TTFT: ${result.timeToFirstToken}ms, "
          "Total: ${result.totalTime}ms, "
          "Tokens: ${result.tokensGenerated}, "
          "Speed: ${result.tokensPerSecond.toStringAsFixed(1)} tok/s",
        );
        debugPrint(
          "[TIMING] Flutter overhead: ${stopwatch.elapsedMilliseconds - result.totalTime}ms",
        );
        debugPrint(
          "[STATUS] Conversation active: ${isConversationActive.value}",
        );
  ...
  void clearConversation() {
    ...
    isConversationActive.value = false;
    debugPrint(
      "Conversation history and native conversation state cleared, interaction mode reset",
    );
  }
  ...
  Future<String> generateSimpleResponse(
    ...
    try {
      return await _cactusContext!.generateResponse(
        userInput,
        maxTokens: maxTokens,
      );
    } catch (e) {
